...
Currently configured for **LangGraph (TypeScript) + React** — the 34 routes of
...
```bash
cd backend && npm run dev                           # http://localhost:8123
...
Videos land in `videos/` as `LANGGRAPH-TS-react-<NN>-<name>.webm`, 1920×1080, ~25fps
...
every demo at `<route>/demo-chat`. This app tracks 36 doc routes; the 2 that carry no
